...
class Game():
    ...
    def redraw(self,x,y,r):
        View_Size = 100 
        screen.fill(background_colour)
        for i in range(len(self.randomlist)):
            color=(self.randomlist[i][0],self.randomlist[i][1],self.randomlist[i][2])
            if(self.randomlist[i][3] > x + View_Size or self.randomlist[i][3] < x - View_Size or self.randomlist[i][4] > y + View_Size or self.randomlist[i][4] < y - View_Size):
                continue
    ...
    def runOtherPlayerBlob(self,fullMsg):
        View_Size = self.player.r * 3
        for i in range(len(fullMsg[1])):
            x,y,r2=fullMsg[1][i][0],fullMsg[1][i][1],fullMsg[1][i][2]
            self.anotherplayer[i].run()
            if(not (self.randomlist[i][3] > x + View_Size or self.randomlist[i][3] < x - View_Size or self.randomlist[i][4] > y + View_Size or self.randomlist[i][4] < y - View_Size)):
                pygame.draw.circle(screen, background_colour, (self.randomlist[i][3],self.randomlist[i][4]), 3)
